...
### 2026-08-30（侧边栏新增固定分组「离线服务器」）

侧边栏「所有服务器」下面多了一个固定分组**离线服务器**，探针掉线的机器会自动进去。点分组标题进入列表视图（新的 `OFFLINE` scope），和区域分组一样。

**它是筛选视图，不是归属**：机器同时还留在自己的区域 / 自定义分组里，不会被「搬走」。

**判定标准**沿用探针那套现成阈值，没有另立一套：探针超过 `推送间隔 × 2 + 60 秒` 没推送就算离线（`probe_offline_after()`）。这个阈值**跟着「连接与通知设置」里的推送间隔走，不是写死的数**：
...
所以间隔调小，掉线发现得就快，代价是面板 CPU 和误报概率（网络抖动一下就可能进组，`×2 + 60` 只兜得住丢一次推送）。徽章上悬停会显示当前阈值是多少分钟，现算的。

**哪些机器参与判定**——这是唯一需要拿主意的地方：

| 机器状态 | 是否进离线组 | 为什么 |
|---|---|---|
| 探针在正常推送 | 否 | 在线 |
| 探针推过、现在超时了 | **是** | 掉线 |
| 标了 `probe_installed` 但一次没推过 | **是** | 装完没跑起来，或主控地址 VPS 侧不通——正是最该被看见的一类 |
| 从没装探针、也从没上报过 | 否 | 我们对它**没有任何观测**，说它「离线」是编的 |

最后一行是刻意的。把未装探针的机器也算离线，会让这个分组在刚导入备份、还没装探针的面板上直接塞满所有机器，分组也就废了。但这类机器不会被悄悄忽略：组里会写一行「另有 N 台未装探针，不参与离线判定」。

**新增的判活函数**（`app/services/probe.py`）：`is_server_monitored()` / `is_server_offline()` / `list_offline_servers()` / `count_unmonitored_servers()`。都是**同步**的——判据和 `get_server_status()` 逐字一致（同一个 `probe_offline_after()`），只是不 `async`，因为它本来就没有 IO，纯读内存里的 `PROBE_DATA_CACHE`，而侧边栏的 `render_sidebar_content` 是同步 refreshable，没法 `await`。

**怎么保持最新**：离线判定纯粹是「探针多久没推」，时间一到自己就变，没有任何事件会通知面板，所以只能定时自查。`main_page` 里挂了个 60 秒的 `ui.timer`（放在 refreshable **外面**，放里面会被自己的 refresh 连带销毁），回调 `refresh_sidebar_if_offline_changed()` 先扫一遍内存缓存，**只在离线集合真的变了时**才 `render_sidebar_content.refresh()`——refresh 会重建所有已连接客户端的侧边栏，无条件每分钟来一次在软路由上就是白烧 CPU。分组渲染时会把当前集合写进快照，所以页面刚打开后的第一次 tick 也不会无谓重建。

**两个容易踩的坑，都处理了**：

- 同一台机器现在在侧边栏出现两次（区域组 + 离线组），而 `SIDEBAR_UI_REFS['rows']` 是按 url 做键的**单值**，`server_dialog` 改完分组后靠它 `row_el.move(target_col)` 搬行。两份都登记会导致后写的赢、搬走的可能是离线组里那一份，机器就从离线组凭空消失。所以 `render_single_sidebar_row()` 加了 `register_ref` 参数，离线组里的副本传 `False`。
- `EXPANDED_GROUPS` / `SIDEBAR_UI_REFS['groups']` 都以分组名为键。固定分组用哨兵键 `__xf_offline__` 而不是「离线服务器」这个显示名，这样用户真建了个同名自定义分组也不会互相顶掉展开状态。

固定分组不参与拖拽排序，所以标题栏里没有拖拽抓手（摆一个是骗人的），光标也从 `cursor-grab` 换成了 `cursor-pointer`；同理它不写进 `custom_groups`，没有「管理分组」按钮。列表视图为空时显示绿色对勾 +「当前没有检测到离线服务器」，而不是像出错的「列表为空」。

**验证**：`py_compile` 四个改动文件 + 40 项渲染 / 判活断言全过——阈值在 4 种间隔下的分档（60s→3 分钟 … 21600s→721 分钟）、同一台静默 5 分钟的机器在 1 分钟间隔下算离线而 30 分钟间隔下仍在线、`>=` 边界（正好到阈值算离线 / 差 1 秒不算）、四种机器状态各自进不进组、徽章计数与阈值 tooltip 的 `target` 确实挂在徽章上、`rows` 引用没被离线组副本顶掉且机器在侧边栏出现两次、`OFFLINE` scope 与 `list_offline_servers()` 取到同一批、定时自查刚渲染完不空转且集合变了只重建一次、哨兵键和同名自定义分组并存互不影响、空面板不说「另有 0 台未装探针」。另外手工核对了 `app.services.probe` 的整条传递依赖链没有模块级 `app.ui.*` 导入（无循环导入），以及刷新页面后 `restore_last_view` 能恢复 `OFFLINE` 视图。
...
原来的结构是「节点池」区块里再套一个「独立节点」小标题。面板节点概览被移除之后，节点池里就只剩独立节点这一样内容了（面板节点的正主是服务器管理页），两级标题说的是同一件事，还各自挂了一个**「批量导入」**——同一个功能在相隔 40px 的地方出现两次。

现在合成一层：区块标题就是「独立节点」，条数 chip 从内层小标题上提到区块标题旁边，批量导入只留标题栏里这一个。空态里那个第三个「批量导入」也去掉了——两个入口就在上面那行标题栏，隔着空框再放一个只是把首屏最大的一块撑得更高。

配套改动：`section_header()` 新增 `count` 参数（给了就在标题旁挂计数 chip）；页头的统计药丸删掉重复的「独立节点」那枚，只留可用节点 / 订阅 / 组合三枚。

**② 订阅链接条：三个重复图标删掉，「⋯」换成带字按钮**

链接条右侧原本是五个 xs 小圆点：`content_copy`（原始链接）/ `bolt`（Surge）/ `cloud_queue`（Clash）/ `qr_code_2` / `more_horiz`。前三个是纯重复——这三种格式在 `more_horiz` 展开的「输出格式」菜单里本来就都有，原始链接更是整条链接条点一下就复制。三个图标既没标签也没颜色区分，用户根本认不出哪个是哪个。

现在只留两个真正独有的入口：**扫码**（`qr_code_2`，二维码里还能切格式）和**输出格式**。菜单入口从省略号换成带字带下拉箭头的按钮——里面躺着 9 种格式（Clash / sing-box / Surge / Quantumult X / Loon / V2Ray-Shadowrocket / ClashR / Shadowsocks / 原始链接），藏在一个 `⋯` 后面等于没有。

顺手修了链接条的复制提示图标：原先它在 `else` 分支里，某些状态下整个不渲染；现在挪进可点击区域内部，常驻。

**③ 区块顺序**

上半部分是独立节点，下半部分是订阅与组合。顺序是刻意的——先有节点才有订阅，从上往下读就是用户的操作顺序。（这一条 2026-08-27 那版就定了，这次只是把标题层级理干净。）

**验证**：`/tmp/subs_render_smoke.py` 在假 client 里把整页真实渲染四遍（有节点有订阅/深色、独立节点为空/深色、订阅为空/浅色、全空/浅色），断言：无「节点池」标题、「独立节点」标题恰好一次、「批量导入」恰好一个、计数 chip 在区块标题上、`cloud_queue`/`more_horiz` 已消失、`bolt` 只剩区块标题那一个、`qr_code_2` 保留、输出格式菜单每张卡 9 项且 Surge / Clash / 原始链接都在、承重功能（添加独立节点 / 编辑 / 普通与组合两组标题 / 三枚药丸）全在、区块顺序正确。四种场景全部通过。

---

### 2026-08-30（主机字段归位：`ssh_host` 只存 IP，域名只存 `cf_primary_domain`）

这版把完整版那套「主机字段归位」整体搬过来，替掉了原先只在编辑框里糊一层的做法。

**要解决的问题**：`ssh_host` 里存着域名时，SSH 连不上机器本身。域名一旦挂在 Cloudflare 橙云后面，普通 DNS 解析出来的是 **CF 边缘 IP**，SSH 打过去必然失败；而探针安装、批量运维全靠 `ssh_host`。

**归位的两步**（`app/services/server_ops.py` 新增）：

1. `backfill_primary_domain(srv)` —— 服务器身上出现过域名（`url` 主机优先，其次 `ssh_host`）而 `cf_primary_domain` 为空时，把域名回填进去，**域名不丢**。`url` 主机优先是为了跟订阅链接现有的回退顺序一致，回填后订阅出站主机不变。
2. `resolve_ssh_host_to_ip(srv)` —— `ssh_host` 是域名时解析成 IP 写回。**先查 Cloudflare 上 A 记录的 `content`（源站 IP）**，拿不到才退回普通 DNS，这是绕开 CF 边缘 IP 的关键。

三条硬约束，都有自测覆盖：

- **解析失败保持原样**，绝不把 `ssh_host` 清空 —— 网络抽一下不该把用户填的主机弄丢。
- **`ssh_host` 本来为空就不动它**，免得给纯 X-UI 服务器凭空造出一份 SSH 配置。
- **`url` 一个字都不改** —— 它是探针上报和节点缓存的主键。

**三个触发点**：

- 打开服务器编辑框（`app/ui/dialogs/server_dialog.py`）：改的是 `SERVERS_CACHE` 里的原对象并落库，所以修正是持久的，不再是「关掉框就白改」。`ssh_host` 为空、输入框要回退显示 `url` 主机时，也先解析成 IP，免得「SSH 主机 IP」里又冒出一个域名。
- **备份导出前**（`app/ui/common/dialogs_data.py`）：带 8 秒预算跑一遍全量归位，保证导出的 JSON 里 `ssh_host` 就是 IP —— 恢复到另一个面板时不会把问题一起搬过去。
- **恢复导入后**：只对本次新增/覆盖的那几台跑归位，老备份里存的域名当场收拾干净。

批量归位 `normalize_all_server_host_fields()` 并发 16、带总时长预算，超时只是提前收工（已改好的照样落库），不会把 UI 卡住。

**跟完整版唯一的实现差异**：完整版用 `asyncio.Lock` 把 CF 查询串起来，因为它的 `CloudflareHandler` 每次查完就关掉内部 httpx client。精简版的 CF 客户端是常驻共享池（`_CLIENTS`）、单次调用不关闭，所以锁去掉了，CF 查询真正并发跑，只保留按域名的记忆缓存。

**顺带修掉 `app/services/domain_sync.py` 两处守卫**：

- 反查域名前加了 `is_ip_literal(current_ip)` 判断。`current_ip` 落成域名时（`url` 里填的就是域名）拿它去查 `list_a_records_by_ip` 必然空手而归，白打一次 CF 往返。
- `ssh_host` 的更新守卫去掉了 `and ssh_host != domain`。原先这个条件的意思是「`ssh_host` 存的正是那个域名时就别动」，方向刚好反了 —— **那才是最该被换成 IP 的情况**。（下面 2026-08-28 那条里提到的「两处更新守卫」，从这版起只剩 `url` 那一处；`url` 的守卫是故意留着的，它是主键。）

`app/utils/network.py` 同时补上 `is_ip_literal` / `extract_host` / `resolve_domain_ip` 三个辅助函数。`resolve_domain_ip` 走 `loop.getaddrinfo`，**不阻塞事件循环**（原先编辑框里那次是裸 `socket.gethostbyname()`，会把整个面板卡住）。`domain_sync.py` 里那个 `_resolve_ip` 故意不改成走缓存版本 —— 它要的就是「域名现在指向哪」，读缓存等于拿旧答案跟旧答案比，IP 变了也发现不了。

### 2026-08-30（订阅管理移除「面板节点」只读概览）

节点池上半部分那块灰底 chip 列表（`服务器名 · 节点数`，98 台铺满一屏）删掉了，只动 `app/ui/pages/subs_page.py` 一个文件。

它是下面 2026-08-27 那次 UI 重构加进来的，当时的理由是「独立节点为空时上半部分不至于是一个空框」。但用下来这个理由站不住：

- **信息是重复的**：节点的正主是服务器管理页，那里有名字、IP、协议、端口、流量。这里只是把同一批数据再列一遍，而且自己就标着「由服务器管理同步，此处只读」——点也点不动。
- **它挤掉了真正要操作的东西**：`max-h-[116px]` 的滚动区被近百个 chip 塞满，把下面「独立节点」那一节连同它的按钮一起顶出首屏，而后者才是这个页面唯一能增删节点的地方。为了填空而加的东西，最后占了比它该占的更多的位置。

**保留不动的**：独立节点的批量导入 / 添加 / 编辑 / 删除全部照旧。这块是承重的——`sub_pipeline.build_node_lookup()` 会把它们以 `independent|<id>` 注册进下发索引，`sub_dialogs` 的节点选择器里也有「独立节点库」分组，是「粘贴机场或自建的分享链接、和面板节点一起打包成订阅」的唯一入口。页头「可用节点 / 独立节点」两个统计药丸保留，节点池副标题的 `面板 N + 独立 M` 计数照旧。

**顺带收掉的死代码**：原先按服务器聚合出 `name / host / total / on` 的那段循环，唯一消费者就是这块 chip 列表。概览删掉后只剩副标题和药丸需要一个总数，于是塌成一行：

```python
panel_total = sum(1 for _node, _host, srv in lookup.values() if srv)
```

`panel_on` 一并删除（除那块 chip 外无人引用）。计数语义没变：新旧写法都等于 `lookup` 里 `srv` 非 None 的条目数，也就是独立节点之外的全部节点。

### 2026-08-28（后台轮询大改：从「定时推」改成「按需拉」）

精简版跑在软路由上，面板容器**持续占 20-42% CPU**、cf-tunnel 另占 8-14%，而日志一行不打。查下来不是 bug，是**完整版的探针节奏搬到了一个不需要探针的面板上**：

- 每台 VPS 的 agent **每 5 秒**推一次，载荷带**整张 x-ui inbound 表**。24 台就是 4.8 req/s、每台每天 17280 次；面板侧每次都无条件做完整解析，而成功路径不打日志——所以表现成「CPU 30% 但日志静默」。
- 域名 IP 同步每小时全量跑一遍 Cloudflare（启动时还多跑一次，就是启动后那串刷屏 CF 日志），而它真正能起作用的场合极少：两处更新守卫保证「url 里填的就是域名」的机器**永远不会被更新**，为它们跑的 CF 往返是纯白费。

精简版只用来管节点，探针指标不需要实时。所以后台轮询整体收敛，**一个功能都没删**：

**探针推送 5 秒 → 可配置，默认 30 分钟。** 「连接与通知设置」里新增下拉项（1 分钟 ~ 6 小时），下面实时算出「判定离线需 61 分钟无推送 / Telegram 告警最慢约 67 分钟」这两个后果，不用自己推。
- 间隔以前硬编码在 agent 脚本里，改一次要 SSH 重装所有 VPS。现在**面板在每次推送的响应体里回传间隔**（`OK 1800`），agent 拿它当下次 `sleep` 的秒数——改间隔就是在面板里改个数字，已装的探针下一轮自己跟上。
- **向后兼容**：老 agent 只看请求成功与否、不读响应体，多出来的数字会被忽略，行为不变。所以可以先只更面板，老的 5 秒 agent 照样正常工作。要拿到「会自调间隔」的 agent，跑一次「一键安装 / 更新所有探针」即可。
- **判活窗口统一从间隔派生**：以前有 5 处硬编码、3 个不同数值（15s / 60s / 20s ×3），谁都不知道以哪个为准。现在全部走 `probe_offline_after()` = `间隔 × 2 + 60`（容忍偶发丢一次推送 + 覆盖 agent 侧 1 秒 CPU 采样与时钟漂移）。这个式子对**任何**间隔都成立，所以灰度期间新面板配旧 agent 只会显得格外新鲜，不会误报离线。
- **陈旧数据如实标注**：指标最长会是半小时前的，详情页现在标出「⏱ 探针数据采集于 X 分钟前」。数字本身没问题，误导人的是隐去年龄。

**域名解析：删掉定时任务，改成打开单机详情页时才同步那一台。** 每小时全量 + 启动一次的 `job_sync_domain_ips` 整个删除，重构成 `app/services/domain_sync.py` 的单台 `sync_server_domain_ip()`（两处更新守卫、netloc 重建、`PROBE_DATA_CACHE` / `NODES_DATA` 键迁移全部逐字保留）。挂在详情页已有的 `load_cloudflare_records` 上——它**本来就在查**「解析本机 IP + `list_a_records_by_ip`」这两步，缺的只是回写，所以不增加任何网络往返。同步失败只记一条 warning，绝不挡住 CF 记录卡片。

**节点数据：推送不再保鲜，补上按需刷新。** 这是本次最容易出错的一环——推送同时也是节点表的运输通道，而原先两个消费方都假定「装了探针就等于数据实时」。改法遵循同一条原则：**自动路径只读缓存，显式动作才走 SSH**。
- 详情页打开时**总是**当场 SSH 直拉节点表（去掉了原来 `not NODES_DATA.get(url)` 那个「只在缓存为空时才拉」的条件）。这正是「主动点开详情页才去解析」用在节点上。
- 列表页保持只读缓存：`NODES_DATA` 是**持久化**的（db 键 `nodes_cache`），重启不丢；节点配置只在用户改动时变，而通过面板改就会 force_refresh 回写。开一次列表页就 SSH 连 24 台，比省下来的推送还贵。
- 按下刷新按钮（`force_refresh=True`）时**把探针机也纳入同步**——以前探针机被无条件排除在外，导致显式刷新对它们其实无效。自动路径（切分组、翻页）行为一个字没动。
- 订阅下发不受影响：`build_node_lookup` 读的就是持久化的 `NODES_DATA`。唯一的窗口是绕过面板直接在 x-ui 里改节点，进一次详情页即刷新。

**Cloudflare 调用效率**（域名解析变交互式之后，这些浪费会直接变成用户打开页面时的等待，所以一并修）：
- **zone 列表加进程级缓存**（TTL 10 分钟）。`get_zone_id` 以前无条件先调一次 `list_zones()`，而打开一次详情页会反复走到它。实测 9 个 CF 方法调用现在只查 1 次 zone。设置页那个「刷新域名」按钮走 `use_cache=False`，用户手点时永远拿实时的；改 token 时缓存作废。
- **不再销毁连接池**：`finally: await self.close()` 出现在 **8 处**，每次调用后把 `httpx.AsyncClient` 连同 TLS 会话一起关掉，下次从 TCP 握手重来（实测每台多 350-700ms）。改成进程级共享客户端，按 (事件循环, token, email) 索引。

**其余同思路的收敛**：详情页两个重复计时器 3 秒 / 2 秒 → 都是 10 秒（刷的全是半小时才变一次的缓存）；仪表盘浏览器轮询 3 秒 → 15 秒（每轮触发一遍 `calculate_dashboard_data()` 全量遍历，已有的 `document.hidden` 跳过与自动停表逻辑保留）；离线监控间隔不动（120 秒，实测每轮 36-53ms，本身不是开销来源），只换掉它依赖的判活窗口；httpx 的 `HTTP Request: ...` INFO 日志静音（CF 一次调用能刷十几行）。

**已知代价，都是明知的取舍**：
- Telegram 离线告警从约 6 分钟变成约 67 分钟（61 分钟窗口 + 连续 3 轮 × 120 秒）。这是「半小时推一次」的必然结果，间隔调小就变快；设置页里直接把这个数算给用户看。
- 指标只有半小时粒度，CPU 曲线基本没有观察价值——符合「精简版不要探针功能」的定位，且已用新鲜度标注避免误读。
- 要跑一次批量更新探针才能拿到会自调间隔的 agent；跑之前一切照旧，不会中断。

**验证**：离线测试 60 余项断言全过——间隔钳位 12 种非法输入、判活窗口在三种间隔下的分档、**老 agent 兼容**（5 秒前的推送在 61 分钟窗口下仍在线）、推送响应带回间隔且错 token 仍 403、agent 端 `parse_interval` 的 11 种响应（含只回 `OK` 时保持上一次的值而非退回高频）、渲染后的安装脚本无残留占位符且 agent 源码能编译、单台域名同步 12 项（url 已是域名不动 / 换 IP 时 url+ssh_host 都更新且两个缓存键都迁移 / 保留 basic auth 与端口 / 无 token 退回系统 DNS / 解析失败不抛异常）、CF zone 缓存 8 项与连接池复用 3 项、Telegram 告警链路仍是「连续 3 轮才报一次 + 恢复通知」、新鲜度文案 11 个边界。

**踩到的坑**：给 agent 的 `parse_interval` 写了个三引号 docstring，直接把面板侧那个原始字符串字面量截断了——整个 agent 脚本是 `PROBE_INSTALL_SCRIPT` 的一部分，里面一个三引号就会让面板直接语法错误起不来。已改成 `#` 注释并在原地写了警告。
